...
class ScriptTreeGenerator {
     ...
     * @param {number} resultType Result type.
     * @param {number} inputType Operand type.
    ...
    descendVariadicOperator (block, prefix, opcode, resultType, inputType) {
        ...
        for (let i = 3; i <= count; i++) {
            result = new IntermediateInput(opcode, resultType, {
                left: result.toType(inputType),
                right: this.descendInputOfBlock(block, `${prefix}${i}`).toType(inputType)
            });
        }
        return result;
    }
    ...
    descendInput (block, preserveStrings = false) {
        ...
        case 'argument_reporter_boolean': {
            ...
            const index = this.script.arguments.lastIndexOf(name);
            if (index === -1) {
                if (name.toLowerCase() === 'is compiled?' || name.toLowerCase() === 'is bilup?') {
        ...
        case 'data_get_list_as':
            return new IntermediateInput(InputOpcode.LIST_AS, InputType.STRING, {
                list: this.descendVariable(block, 'LIST', LIST_TYPE),
                format: block.fields.FORMAT?.value || block.fields.TYPE?.value || 'JSON'
            });
        ...
        case 'looks_costumes':
            return new IntermediateInput(InputOpcode.LOOKS_COSTUMES, InputType.STRING);
        ...
        case 'operator_add':
            return this.descendVariadicOperator(
                block, 'NUM', InputOpcode.OP_ADD, InputType.NUMBER_OR_NAN, InputType.NUMBER
            );
        case 'operator_and':
            return this.descendVariadicOperator(
                block, 'OPERAND', InputOpcode.OP_AND, InputType.BOOLEAN, InputType.BOOLEAN
            );
        ...
        case 'operator_divide':
            return this.descendVariadicOperator(
                block, 'NUM', InputOpcode.OP_DIVIDE, InputType.NUMBER_OR_NAN, InputType.NUMBER
            );
        ...
        case 'operator_join':
            return this.descendVariadicOperator(
                block, 'STRING', InputOpcode.OP_JOIN, InputType.STRING, InputType.STRING
            );
        ...
        case 'operator_letters_of':
            return new IntermediateInput(InputOpcode.OP_LETTERS_OF, InputType.STRING, {
                start: this.descendInputOfBlock(block, 'LETTER1').toType(InputType.NUMBER),
                end: this.descendInputOfBlock(block, 'LETTER2').toType(InputType.NUMBER),
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING)
            });
        case 'operator_index_of':
            return new IntermediateInput(InputOpcode.OP_INDEX_OF, InputType.NUMBER_WHOLE, {
                substring: this.descendInputOfBlock(block, 'SUBSTRING').toType(InputType.STRING),
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING)
            });
        case 'operator_change_case':
            return new IntermediateInput(InputOpcode.OP_CHANGE_CASE, InputType.STRING, {
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING),
                upper: block.fields.CASE.value === 'uppercase'
            });
        case 'operator_repeat':
            return new IntermediateInput(InputOpcode.OP_REPEAT, InputType.STRING, {
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING),
                count: this.descendInputOfBlock(block, 'REPEAT').toType(InputType.NUMBER)
            });
        case 'operator_replace':
            return new IntermediateInput(InputOpcode.OP_REPLACE, InputType.STRING, {
                substring: this.descendInputOfBlock(block, 'SUBSTRING').toType(InputType.STRING),
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING),
                replacement: this.descendInputOfBlock(block, 'REPLACE').toType(InputType.STRING)
            });
        case 'operator_trim':
            return new IntermediateInput(InputOpcode.OP_TRIM, InputType.STRING, {
                string: this.descendInputOfBlock(block, 'STRING').toType(InputType.STRING)
            });
        case 'operator_pi':
        ...
        case 'operator_mod':
            return this.descendVariadicOperator(
                block, 'NUM', InputOpcode.OP_MOD, InputType.NUMBER_OR_NAN, InputType.NUMBER
            );
        case 'operator_multiply':
            return this.descendVariadicOperator(
                block, 'NUM', InputOpcode.OP_MULTIPLY, InputType.NUMBER_OR_NAN, InputType.NUMBER
            );
        case 'operator_not':
        ...
        case 'operator_or':
            return this.descendVariadicOperator(
                block, 'OPERAND', InputOpcode.OP_OR, InputType.BOOLEAN, InputType.BOOLEAN
            );
        ...
        case 'operator_subtract':
            return this.descendVariadicOperator(
                block, 'NUM', InputOpcode.OP_SUBTRACT, InputType.NUMBER_OR_NAN, InputType.NUMBER
            );
    ...
    descendStackedBlock (block) {
        ...
        case 'data_set_list_to_array':
            return new IntermediateStackBlock(StackOpcode.LIST_SET_ARRAY, {
                list: this.descendVariable(block, 'LIST', LIST_TYPE),
                array: this.descendInputOfBlock(block, block.inputs.ARRAY ? 'ARRAY' : 'VALUE').toType(InputType.STRING)
            });
        ...
        case 'motion_pointtowards_xy':
            return new IntermediateStackBlock(StackOpcode.MOTION_POINT_TOWARDS_XY, {
                x: this.descendInputOfBlock(block, 'X').toType(InputType.NUMBER),
                y: this.descendInputOfBlock(block, 'Y').toType(InputType.NUMBER)
            });
        case 'motion_pointtowards_xyfrom':
            return new IntermediateStackBlock(StackOpcode.MOTION_POINT_TOWARDS_XY_FROM, {
                x: this.descendInputOfBlock(block, 'X').toType(InputType.NUMBER),
                y: this.descendInputOfBlock(block, 'Y').toType(InputType.NUMBER),
                fromX: this.descendInputOfBlock(block, 'FROMX').toType(InputType.NUMBER),
                fromY: this.descendInputOfBlock(block, 'FROMY').toType(InputType.NUMBER)
            });
        ...
        case 'pen_printText':
            return new IntermediateStackBlock(StackOpcode.PEN_PRINT_TEXT, {
                text: this.descendInputOfBlock(block, 'TEXT').toType(InputType.STRING),
                x: this.descendInputOfBlock(block, 'X').toType(InputType.NUMBER),
                y: this.descendInputOfBlock(block, 'Y').toType(InputType.NUMBER)
            });
        case 'pen_drawTriangle':
            return new IntermediateStackBlock(StackOpcode.PEN_DRAW_TRIANGLE, {
                x0: this.descendInputOfBlock(block, 'X0').toType(InputType.NUMBER),
                y0: this.descendInputOfBlock(block, 'Y0').toType(InputType.NUMBER),
                x1: this.descendInputOfBlock(block, 'X1').toType(InputType.NUMBER),
                y1: this.descendInputOfBlock(block, 'Y1').toType(InputType.NUMBER),
                x2: this.descendInputOfBlock(block, 'X2').toType(InputType.NUMBER),
                y2: this.descendInputOfBlock(block, 'Y2').toType(InputType.NUMBER)
            });
        ...
        default: {
            ...
            if (opcodeFunction) {
                ...
                if (asVisualReport) return asVisualReport;
                return this.descendCompatLayerStack(block);
            }
    ...
    /**
     * Preserve the source-splicing behavior used by legacy Mist's Utils projects.
     * Literal text is JavaScript source; reporter inputs are compiled expressions.
     * @param {*} block Patch block.
     * @returns {IntermediateInput[]}
     */
    ...
    /**
     * @param {*} block Patch reporter block.
     * @param {number} type Compiler input type.
     * @returns {IntermediateInput}
     */
    descendRawSourceInput (block, type) {
        return new IntermediateInput(InputOpcode.RAW_SOURCE, type, {
            fragments: this.descendRawSourceFragments(block)
        });
    }

    descendCompilerExtensionInput (block, compiler) {
        const inputs = {};
        for (const name of Object.keys(block.inputs)) {
            inputs[name] = this.descendInputOfBlock(block, name, true);
        }
    ...
    descendCompilerExtensionStack (block, compiler) {
        const input = this.descendCompilerExtensionInput(block, Object.assign({}, compiler, {type: 'any'}));
        return new IntermediateStackBlock(StackOpcode.EXTENSION, input.inputs);
    }
